...
> - `window.py` `_on_crabcards_extracted` now stores both `session_key` AND `tab_key` on cards
> - `feed_handler.py` `_maybe_create_snapshot` uses `tab_key` for chat box lookup (falls back to `session_key`)
> - `chat_handler.py` `render_sync` now passes `tab_key=tab` to the render pipeline
> - Snapshot creation works correctly for both project group chats and direct chats
