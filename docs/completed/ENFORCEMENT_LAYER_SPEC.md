...
> - `agent/enforcement.py` exists and active
> - `agent/runtime.py` calls `_enforcement_check()` on write_file/edit_file operations
> - `ui/handlers/agent_runtime_handler.py` has `_on_enforcement_status` callback
> - Enforcement config in `agent/config.py`
