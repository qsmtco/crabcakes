...
| **Conversation creation on first send** | `_ensure_system_prompt` runs in background thread. User sees "thinking..." immediately. | The first `on_text_delta` is the BUG #21 empty-string turn-start signal — fires before prompt build completes? Wait, no: the prompt build happens in `_run_loop` BEFORE the LLM call. The empty-string turn-start signal is dispatched after `_ensure_system_prompt` returns. So the user sees nothing until both the prompt is built AND the first LLM token arrives. **Mitigation:** add `_on_agent_start_cb(session_key)` call at the very start of `_run_loop` (before `_ensure_system_prompt`), so the activity indicator shows immediately. |
