...
- **`get_tools_for_api` rewrite** — verified the exact current code at `mcp_client.py:520`. The only change is `namespaced = f"{server_name}/{tool.name}"` → `wire_name = _to_wire_name(server_name, tool.name)`. The rest of the loop body is unchanged.
