...
| #9 | HIGH | Persistence test uses same session key for 4 terminal transitions (impossible — dedup prevents 2nd+) | §2.6 group 2 (`test_terminate_turn_persistence_uses_separate_session_keys` + `test_terminate_turn_cancelled_with_persist_metadata_saves`) | Test rewritten to use 3 separate session keys (one per terminal status). New acceptance criterion verifies the deduplication behavior. Group 2 expanded from 6 to 8 tests total. |
