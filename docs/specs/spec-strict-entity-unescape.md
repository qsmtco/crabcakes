...
**Source files read:**
- `utils/escaping.py` (full, 219 lines) — `escape_for_pango` lives here. Line 78 calls `text = html.unescape(text)` at the start of the function. The comment above (lines 74-77) says: "Decode HTML entities that LLMs sometimes emit (&quot;, &amp;, &lt;, etc.) before processing. Without this, html.escape() below would double-encode them (e.g. &quot; → &amp;quot;) and they'd appear as raw text in bubbles." Lines 85-156 contain the main loop (stack-based tag parser). Line 108 has the closing-tag regex `</([a-zA-Z][a-zA-Z0-9._-]*)\s*>`. Line 125 has the opening-tag regex `<([a-zA-Z][a-zA-Z0-9._-]*)([^>]*)>`. Line 146 has the attribute-escape regex `&(?![a-zA-Z#0-9]+;)` that escapes bare `&` inside known-tag attribute values.
