...
**2.3.2 Retention rules inside `compact_feed`:** a card is **pinned** (never pruned) if any of: `accepted is not None`; `needs_review`/`needs_approval` **while genuinely undecided**; `card_type == "git_commit"`.

**"Genuinely undecided" (F1 amendment, 2026-09-12):** a `needs_review`/`needs_approval` card pins ONLY when no decision is recorded anywhere: `card.accepted is None` AND `metadata.get("status") not in ("approved", "denied")`. Rationale (external verification `docs/specs/UIRESP2-PHASE1-3-VERIFICATION-QRUSHER.md` F1): `needs_approval` is a transient creation-time flag that is never cleared, so pinning on its historical presence pins every exec-approval card forever (measured live: 2,658 pinned-outside-window → retention 4,658 instead of 2,000, growing ~350/day). `approve_exec` writes `status="approved"/"denied"` into `metadata` (persisted via the update payload) and `accepted` in memory — the durable decision record therefore exists in `metadata.status`, and the pin rule reads it. Companion fix: `update_card`'s enqueue payload additionally includes `"accepted": card_data.accepted` **when not None**, so future decisions persist the durable field too (closing the accepted-never-persisted gap; pre-existing — the pre-Phase-1 sync path had the same payload). The transient flag is deliberately NOT popped on resolution (UI rendering keys on it; changing the resolved-card widget is out of scope). Keep the newest `window` by list order (`[-window:]`), then additionally pin any card outside that slice matching the rules. Prune the rest. Log at WARNING.
