...
2. **Feeds stop growing without bound.** Compaction retains the newest 2,000 cards (actionable/accepted/git-commit cards pinned forever); a 13.9 MB / 9,500-card feed compacts to the window on first open after upgrade, with a visible "Feed compacted — N oldest cards pruned" system card; loads then complete in <100 ms (tested). Code path: `_load_and_render` trigger → writer compact branch → `_surface_prune_card` (main-thread name resolution, persisted copy).
