...
class TestPopoverLeakGuard:
    """BUG #1 (adversarial audit): show_suggestions_menu must dismiss any
    previous popover before creating a new one, and clear its ref on
    'closed', to prevent popover leaks on repeated right-clicks.
    """

    def test_suggestion_popover_attr_initialized_to_none(self):
        """_suggestion_popover is None before any call to show_suggestions_menu."""
        toolbar = ChatInputToolbar()
        assert toolbar._suggestion_popover is None

    def test_second_call_dismisses_previous_popover(self):
        """Calling show_suggestions_menu twice dismisses the first popover.

        Failure-case reproduction: if the cleanup guard is missing, both
        popovers are parented to the same widget simultaneously (leak).
        """
        toolbar = ChatInputToolbar()
        parent = Gtk.Box()
        toolbar.show_suggestions_menu(["alpha"], lambda s: None, parent_widget=parent)
        first = toolbar._suggestion_popover
        assert first is not None
        assert first.get_parent() is parent, "first popover should be parented to `parent`"

        toolbar.show_suggestions_menu(["beta"], lambda s: None, parent_widget=parent)
        second = toolbar._suggestion_popover

        # The first popover must have been unparented
        assert first.get_parent() is None, (
            "first popover leaked: still parented after second show_suggestions_menu call"
        )
        # The second popover is a new object, parented correctly
        assert second is not first
        assert second.get_parent() is parent

    def test_closed_signal_clears_suggestion_popover_ref(self):
        """The 'closed' handler clears self._suggestion_popover."""
        toolbar = ChatInputToolbar()
        parent = Gtk.Box()
        toolbar.show_suggestions_menu(["x"], lambda s: None, parent_widget=parent)
        popover = toolbar._suggestion_popover
        assert popover is not None
        # Emit the 'closed' signal manually
        popover.emit("closed")
        assert toolbar._suggestion_popover is None, (
            "closed signal did not clear _suggestion_popover — next call will leak again"
        )

    def test_second_call_after_closed_succeeds_without_crash(self):
        """Sad path: popover was already closed; calling show_suggestions_menu
        again must not crash (cleanup guard handles None / detached prev)."""
        toolbar = ChatInputToolbar()
        parent = Gtk.Box()
        toolbar.show_suggestions_menu(["x"], lambda s: None, parent_widget=parent)
        # Simulate the popover being closed externally
        toolbar._suggestion_popover.emit("closed")
        assert toolbar._suggestion_popover is None
        # Now call again — should not raise
        toolbar.show_suggestions_menu(["y"], lambda s: None, parent_widget=parent)
        assert toolbar._suggestion_popover is not None
        assert toolbar._suggestion_popover.get_parent() is parent


# ── Bug fix regression tests (BUG #2: test coverage for new code paths) ──


class TestPopoverCodePaths:
    """BUG #2 (adversarial audit): the deferred-popup and pointing_to code
    paths added in the popover-positioning fix had ZERO test coverage.
    `get_root()` returns None in headless tests, so the GLib.idle_add
    block was dead code in tests. These tests mock get_root to non-None
    to actually exercise the new branches.
    """

    def test_pointing_to_rect_is_applied_to_popover(self):
        """Test 1: pointing_to=(10,20,30,40) reaches set_pointing_to as a
        Gdk.Rectangle with matching x/y/width/height.

        Failure-case: if the new `if pointing_to is not None` branch is
        removed, `set_pointing_to` is never called and the arrow points
        at the parent widget's origin, not at the clicked word.
        """
        toolbar = ChatInputToolbar()
        parent = Gtk.Box()
        with patch("ui.views.chat_input_toolbar.Gtk.Popover.set_pointing_to") as mock_spt:
            toolbar.show_suggestions_menu(
                ["x"],
                lambda s: None,
                parent_widget=parent,
                pointing_to=(10, 20, 30, 40),
            )
            # The popover was created and parented
            assert toolbar._suggestion_popover is not None
            assert toolbar._suggestion_popover.get_parent() is parent
            # set_pointing_to must have been called with a Gdk.Rectangle
            assert mock_spt.called, (
                "set_pointing_to was not called when pointing_to was provided"
            )
            rect = mock_spt.call_args[0][0]
            assert isinstance(rect, Gdk.Rectangle), (
                f"expected Gdk.Rectangle, got {type(rect).__name__}"
            )
            assert rect.x == 10
            assert rect.y == 20
    ...
    def test_deferred_popup_via_glib_idle_add_when_parent_widget_set(self):
        ...
        toolbar = ChatInputToolbar()
        parent = Gtk.Box()
        with patch.object(toolbar, "get_root", return_value=Gtk.Window()), \
             patch("ui.views.chat_input_toolbar.GLib.idle_add") as mock_idle, \
             patch("ui.views.chat_input_toolbar.Gtk.Popover.popup") as mock_popup:
            toolbar.show_suggestions_menu(
                ["x"],
                lambda s: None,
                parent_widget=parent,
    ...
    def test_direct_popup_when_no_parent_widget_and_root_exists(self):
        """Test 3: parent_widget=None + get_root() != None → popover.popup()
        is called directly and GLib.idle_add is NOT called.

        Failure-case: if the else-branch (direct popup) is missing or
        changed to also defer, the backward-compat path breaks.
        """
        toolbar = ChatInputToolbar()
        with patch.object(toolbar, "get_root", return_value=Gtk.Window()), \
             patch("ui.views.chat_input_toolbar.GLib.idle_add") as mock_idle, \
            ...
            # Direct popup path must have been taken
            assert mock_popup.called, (
                "popover.popup() was not called in backward-compat path"
            )
            # Deferral must NOT have been used
            assert not mock_idle.called, (
                "GLib.idle_add was called in backward-compat path — wrong branch"
            )

    def test_no_popup_when_root_is_none(self):
        """Test 4: get_root() returns None → no popup call at all. The
        popover is still created (and tracked in _suggestion_popover)
        so tests can inspect its structure.

        Failure-case: if the `if self.get_root() is not None:` guard is
        removed, test environments would crash trying to map a popover
        with no display server, or production would try to popup during
        construction before the widget is realized.
        """
        toolbar = ChatInputToolbar()
        # No mocking of get_root — it returns None naturally in headless
        with patch("ui.views.chat_input_toolbar.GLib.idle_add") as mock_idle, \
             patch("ui.views.chat_input_toolbar.Gtk.Popover.popup") as mock_popup:
            toolbar.show_suggestions_menu(
                ["x"],
                lambda s: None,
            )
            # The popover WAS created and tracked
            assert toolbar._suggestion_popover is not None, (
                "popover was not created — widget structure is incomplete"
            )
            # But it was NOT mapped
            assert not mock_idle.called, (
                "GLib.idle_add was called even though get_root() is None"
            )
            assert not mock_popup.called, (
                "popover.popup() was called even though get_root() is None"
            )
